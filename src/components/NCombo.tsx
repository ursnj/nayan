    ...
    <Combobox value={selected} onChange={props.onSelected} {...(Array.isArray(selected) ? ({ multiple: true } as any) : {})}>
      <div className="relative">
        <div className="relative w-full cursor-default overflow-hidden rounded text-left">
          <Combobox.Input
            className="nyn-combo-input w-full py-2 pl-3 pr-10 text-sm leading-5 card-background border rounded text focus:ring-0"
              ...
              <Combobox.Option
                ...
                {({ selected }) => (
                  <>
                    ...
                    {selected ? (
                      <span className="nyn-combo-option-icon absolute inset-y-0 left-0 flex items-center pl-3 text-primary">
